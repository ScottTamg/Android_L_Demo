package com.example.test.myapplicationdemo.util;
...
public class CommonUtil {
    ...
    public static void installApk(Context context, File file) {
        ...
        intent.addCategory("anddroid.intent.category.DEFAULT");
    ...
    public static String collectDeviceInfoStr(Context context) {
        ...
        for (Iterator iter = deviceInfos.iterator(); iter.hasNext();) {
            ...
            deviceInfoStr.append("\t\t\t" + item + ":" + properties.get(iter) + ", \n");
        }
    ...
    public static int getTopBarHeight(Activity activity) {
        return activity.getWindow().findViewById(Window.ID_ANDROID_CONTENT).getId();
    }
     ...
     * @param isFormal 时间数字是否要格式化，如果true：少位数前面补全；如果false：少位数前面不补全。
     * @return 返回时间字符串：小时/分/秒/毫秒的格式（如：24903600 --> 06小时55分03秒600毫秒）。
     * 格式为小时/分/秒/毫秒（如：24903600 –> 06小时55分03秒600毫秒）
     */
    public static String millisToString(long millis, boolean isWhole, boolean isFormal) {
        String h = "";
        String m = "";
        String s = "";
        String mi = "";
    ...
    public static String millisToLifeString(long millis) {
        ...
        //一小时内
        if (now - millis <= oneHourMillis && now - millis > 01) {
